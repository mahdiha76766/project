...
export function AdminImageUploader({
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    const batch = multiple ? list.slice(0, Math.max(remaining, 0)) : [list[0]];
    if (!batch.length) {
      setError(`حداکثر ${maxFiles.toLocaleString('fa-IR')} تصویر مجاز است.`);
      return;
    }
    ...
    for (const file of batch) {
      ...
      const { ok, data, error: uploadError } = await adminFetch<{ url: string }>('/api/admin/upload', {
        method: 'POST',
        body: fd
      });
      if (!ok) {
    ...
    <div>
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2 py-3 text-center">
          ...
          <p className="text-sm font-semibold text-slate-700">تصویر را بکشید و رها کنید یا انتخاب کنید</p>
          <p className="text-xs text-slate-500">JPG, PNG, WEBP, GIF — حداکثر ۳ مگابایت</p>
          <button
            type="button"
            onClick={() => inputRef.current?.click()}
            disabled={uploading || disabled || (multiple && value.length >= maxFiles)}
      ...
      {value.length ? (
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {value.map((url, index) => (
            <div key={`${url}-${index}`} className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="absolute inset-0 flex items-end justify-between gap-1 bg-gradient-to-t from-black/60 to-transparent p-2 opacity-0 transition group-hover:opacity-100">
                {multiple && index > 0 ? (
                  <button
                    ...
                    className="rounded-lg bg-white/90 px-2 py-1 text-[10px] font-bold text-slate-700"
                  >
                    تصویر اصلی
                  </button>
                ) : (
