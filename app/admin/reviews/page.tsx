...
export default function AdminReviewsPage(){
  ...
  <div className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-3 xl:grid-cols-6">
    <input className="h-11 rounded-xl border border-slate-200 px-3" placeholder="Product ID" value={filters.productId} onChange={(e)=>setFilters({...filters,productId:e.target.value})}/>
  ...
  <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="bg-slate-50 text-slate-600"><tr className="[&>th]:px-4 [&>th]:py-3 text-right"><th>کاربر</th><th>محصول</th><th>امتیاز</th><th>وضعیت</th><th>تاریخ</th><th>متن</th><th>پاسخ ادمین</th><th>عملیات</th></tr></thead><tbody className="divide-y divide-slate-100">{items.map(r=><tr key={r._id} className="[&>td]:px-4 [&>td]:py-3 align-top"><td>{r.userId?.name||r.userName||r.userId?.mobile||'-'}</td><td>{r.productId?.name||'-'}</td><td>{r.rating}</td><td>{r.status==='PENDING'?'در انتظار تایید':r.status==='APPROVED'?'تایید شده':'رد شده'}</td><td>{new Date(r.createdAt).toLocaleDateString('fa-IR')}</td><td><div className="max-w-xs"><p className="font-semibold">{r.title}</p><p className="text-xs text-slate-600 whitespace-pre-wrap">{r.comment}</p></div></td><td><input className="h-10 w-full rounded-xl border border-slate-200 px-3" defaultValue={r.adminReply||''} onBlur={(e)=>{ if(e.target.value !== (r.adminReply||'')) void updateReview(r._id,{adminReply:e.target.value},'ثبت پاسخ ادمین انجام شود؟'); }} /></td><td className="space-y-1"><button className="block text-emerald-700" onClick={()=>void updateReview(r._id,{action:'approve'},'این نظر تایید شود؟')}>تایید</button><button className="block text-orange-700" onClick={()=>void updateReview(r._id,{action:'reject'},'این نظر رد شود؟')}>رد</button><button className="block text-amber-700" onClick={()=>{ const title=prompt('عنوان جدید',r.title)||r.title; const comment=prompt('متن جدید',r.comment)||r.comment; void updateReview(r._id,{title,comment},'نظر ویرایش شود؟'); }}>ویرایش</button><button className="block text-red-600" onClick={()=>void updateReview(r._id,{action:'soft_delete'},'حذف نرم انجام شود؟')}>حذف نرم</button></td></tr>)}</tbody></table></div></div>
  </main>;
}
